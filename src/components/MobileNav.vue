<script lang="ts" setup>
import { ref } from 'vue'
import ColorModePicker from './ColorModePicker.vue';

defineProps({
    navLinks: Array
});

const isShown = ref(false);

const handleToggleButtonClick = () => {
    if (isShown.value) {
        document.body.style.overflow = 'auto'
    } else {
        document.body.style.overflow = 'hidden'
    }
    isShown.value = !isShown.value;
}
</script>

<template>
    <div class="mobile-nav w-[24px] sm:hidden">
        <div class="flex">
            <button class="w-full my-auto" @click="handleToggleButtonClick">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                    className="h-8 w-8 text-gray-900 dark:text-gray-100">
                    <path fillRule="evenodd"
                        d="M3 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 15a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z"
                        clipRule="evenodd" />
                </svg>
            </button>
        </div>
        <div class="fixed left-0 top-0 z-10 h-full w-full transform bg-white opacity-95 duration-300 ease-in-out dark:bg-gray-950 dark:opacity-[0.98]"
            :class="[isShown ? 'translate-x-0' : 'translate-x-full']">
            <div class="flex justify-end">
                <button class="mr-8 mt-11 h-8 w-8" @click="handleToggleButtonClick">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                        className="text-gray-900 dark:text-gray-100">
                        <path fillRule="evenodd"
                            d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                            clipRule="evenodd" />
                    </svg>
                </button>
            </div>
            <nav class="fixed mt-8 h-[80%] px-12 py-4 flex flex-col">
                <a v-for="link in navLinks" :key="link.title" :href="link.href"
                    class="block mb-2 text-2xl font-bold tracking-widest text-gray-900 dark:text-gray-100"
                    @click="handleToggleButtonClick">
                    {{ link.title }}
                </a>
                <div class="control-buttons flex gap-4 mt-auto">
                    <ColorModePicker />
                </div>
            </nav>
        </div>
    </div>
</template>